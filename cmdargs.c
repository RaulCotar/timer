	...
	for (int a=1; a<argc; a++) {
		...
		for (int f=1; argv[a][f]; f++) {
			...
			case 'R':
				...
				default: panic("Unknown resolution option '%c'", argv[a][f]); return -2;
				}
				break;
			case 'r':
...
static char const *const help_msg =
	...
	"  -c, --cmd=\x1b[3mCMD\x1b[23m\t\t\trun the given command in sh at program exit\n"
	...
	"\x1b[3mTIME\x1b[23m is specified as: \x1b[3m([[:uint:]][hmsl])+\x1b[23m\n\n"
	"Examples:\n"
